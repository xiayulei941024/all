<template>
  <pro-layout
    ...
    :class="className"
    :header-action-render="tabActionRender"
    @menu-header-click="toHome"
  >
    <template #logo>
      <img src="@/assets/logo.png" alt="logo" :style="{ height: collapsed ? '16px' : '28px', marginLeft: '28px' }" />
    </template>

    <template #rightContentRender>
      <right-content />
    </template>
    <template #menuFooterRender></template>
    ...
    <!-- 当前激活页签处理 -->
    <template #activeTabRender="props">
      <active-tab v-bind="props" />
    </template>
...
import ActiveTab from '@/layouts/components/ActiveTab/index'
...
watch(
  ...
  () => {
    console.log('Store.setting', settingStore.setting)
  },
  { deep: true }
)
...
const tabActionRender = route => {
  // if (route.path === '/news-publish-list') {
  //   return h(
  //     'span',
  //     {
  //       onClick() {
