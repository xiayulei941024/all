<template>
  ...
  >
    <template #headerTitle>
      <a-space :size="24">
        组织架构
        <a-input-search v-model:value="searchName" placeholder="组织名称" allow-clear @search="searchTable" />
      </a-space>
    </template>
    ...
    <template #bodyCell="{ column, record }">
      ...
      <template v-else-if="column.key === 'operate'">
        <operation-group>
          ...
          <delete-text-button v-if="hasPermission('system:organization:del')" @confirm="handleDelete(record)" />
        </operation-group>
      </template>
    </template>
  </pro-table>
  ...
  <!-- 系统组织新建编辑的表单弹窗 -->
  <sys-organization-form-modal ref="fromModalRef" :organization-tree="organizationTree" @submit-success="reloadTable" />
</template>
...
import { listOrganizations, deleteOrganization, revisedOrganization } from '@/api/system/organization'
