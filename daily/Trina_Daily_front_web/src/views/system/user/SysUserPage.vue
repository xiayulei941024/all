  ...
  <a-row :gutter="14">
    <!-- <a-col :xs="24" :md="8" :lg="5">
      <a-card :body-style="{ padding: 0 }" :bordered="false">
        <sys-organization-search-tree :style="organizationTreeStyle" @select="onOrganizationSelect" />
      </a-card>
    </a-col> -->
          ...
          >
                ...
                <a-dropdown v-if="hasPermission('system:user:edit')">
                  ...
                  <template #overlay>
                    <a-menu @click="(info: MenuInfo) => handleUpdateStatus(selectedRowKeys, info)">
                      <a-menu-item :key="1"> <DeleteOutlined style="margin-right: 8px" />开启 </a-menu-item>
                      <a-menu-item :key="0"> <LockOutlined style="margin-right: 8px" />锁定 </a-menu-item>
                    </a-menu>
                  </template>
                </a-dropdown>
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'operate'">
                <TableOperateButton :action-btns="actionBtns" :record="record" />
              </template>
            </template>
          </pro-table>
        </div>
...
const actionBtns = ref([
  ...
  {
    text: '授权',
    permissions: ['system:user:grant'],
    event: handleGrant,
  },
  // {
  //   text: '改密',
  //   permissions: ['system:user:pass'],
  //   event: changePass
  // },
  {
    text: '删除',
